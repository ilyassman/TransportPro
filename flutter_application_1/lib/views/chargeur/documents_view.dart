...
class ReservationDocument {
  ...
  factory ReservationDocument.fromJson(Map<String, dynamic> json) {
    // Construire les informations du camion
    String camionInfo = '';
    if (json['marque'] != null && json['modele'] != null) {
      camionInfo = '${json['marque']} ${json['modele']}';
      if (json['capacite'] != null) {
        camionInfo += ' • ${json['capacite']} tonnes';
      }
    } else {
      camionInfo = 'Camion non assigné';
    }

    // Construire les informations du transporteur
    String transporteurName = json['transporteurNom'] ?? 'Transporteur non assigné';
    if (json['transporteurCompany'] != null && json['transporteurCompany'].toString().isNotEmpty) {
      transporteurName = json['transporteurCompany'].toString();
    }
...
class _DocumentsViewState extends State<DocumentsView> {
  ...
  List<ReservationDocument> _allDocuments = [];
  ...
  Future<void> _loadRecapitulatifs() async {
    try {
      ...
      final documents = data.map((json) => ReservationDocument.fromJson(json)).toList();
  ...
  List<ReservationDocument> get _filteredDocuments {
    return _allDocuments.where((doc) {
      return doc.reservationId.toLowerCase().contains(_searchQuery.toLowerCase()) ||
             doc.transporteurName.toLowerCase().contains(_searchQuery.toLowerCase()) ||
  ...
  void _downloadDocument(ReservationDocument document) async {
    try {
  ...
  void _viewDocumentDetails(ReservationDocument document) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        ...
        content: SingleChildScrollView(
          child: Column(
            ...
            children: [
              Text('Transporteur: ${document.transporteurName}'),
              const SizedBox(height: 8),
              Text('Camion: ${document.camionInfo}'),
              const SizedBox(height: 8),
              ...
              Text(
                'Le PDF contiendra toutes les informations détaillées de la réservation, du transporteur et du camion.',
                style: TextStyle(
                  fontSize: 12,
    ...
    return Container(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          hintText: 'Rechercher par réservation, transporteur ou trajet...',
        ...
        children: [
          ...
          Expanded(
            child: Column(
              ...
              children: [
                Text(
                  'Récapitulatifs de Réservations',
                  style: const TextStyle(
                    fontWeight: FontWeight.bold,
                    fontSize: 16,
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            Text(
              _allDocuments.isEmpty ? 'Aucun récapitulatif disponible' : 'Aucun récapitulatif trouvé',
              style: TextStyle(
                fontSize: 18,
                color: Colors.grey[600],
                fontWeight: FontWeight.w500,
            ...
            if (_allDocuments.isEmpty) ...[
              const SizedBox(height: 8),
              Text(
                'Vos récapitulatifs apparaîtront ici après vos premières réservations',
                style: TextStyle(
                  fontSize: 14,
